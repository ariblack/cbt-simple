...
// ─── HELPER: normalisasi ujian dari db ─────────────────────────────────────────
// Jika db.json masih menyimpan ujian sebagai array (format lama [{...}]),
// langsung migrasi ke object dan tulis ke db agar semua db.set(ujian.xxx) aman.
function getUjianConfig() {
    ...
    if (Array.isArray(raw)) {
        ...
        console.log('[DB] Migrasi ujian array to object selesai.');
        flushCache();
        return normalized;
    }
    return raw || { paket: [] };
...
app.post('/simpan-semua-ujian', (req, res) => {
    try {
        ...
        // Migrasi otomatis dipastikan sudah terjadi di dalam getUjianConfig()
        getUjianConfig();
