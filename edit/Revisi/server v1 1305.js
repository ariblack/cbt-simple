...
// ─── HELPER: normalisasi ujian dari db (bisa array atau object) ───────────────
function getUjianConfig() {
    const raw = db.get('ujian').value();
    // db.json lama menyimpan ujian sebagai array [{paket:[...], ...}]
    // versi baru menyimpan sebagai object {paket:[...], ...}
    return Array.isArray(raw) ? (raw[0] || { paket: [] }) : (raw || { paket: [] });
}

// ─── HELPER: cek apakah paket aktif berdasarkan waktu server real-time ────────
function isPaketAktifSekarang(paket) {
    // 1. Harus di-set aktif oleh admin
    if (!paket.aktif) return false;

    const now = new Date();

    // 2. Cek tanggal ujian (opsional — jika diisi harus cocok)
    if (paket.tanggalUjian) {
        // tanggalUjian format: "YYYY-MM-DD"
        const [ty, tm, td] = paket.tanggalUjian.split('-').map(Number);
        const today = new Date();
        if (today.getFullYear() !== ty || today.getMonth() + 1 !== tm || today.getDate() !== td) {
            return false; // Beda hari
        }
    }

    // 3. Cek rentang jam mulai–selesai (format "HH:MM" atau "HH:MM:SS")
    if (paket.jamMulai && paket.jamSelesai) {
        ...
        if (nowMenit < mulaiMenit || nowMenit >= selesaiMenit) {
            return false; // Di luar jam ujian
        }
    }
...
app.post('/simpan-semua-ujian', (req, res) => {
    try {
        ...
        // ── PERBAIKAN: normalisasi db ujian ke object jika masih array ────────────
        const rawUjian = db.get('ujian').value();
        if (Array.isArray(rawUjian)) {
            // Migrasi sekali: konversi array → object dan simpan ke db
            const existing = rawUjian[0] || {};
            const normalized = {
                paket: existing.paket || [],
                tampilkanNilai: existing.tampilkanNilai !== undefined ? existing.tampilkanNilai : true,
                maxPelanggaran: existing.maxPelanggaran || 3,
                pesanPelanggaran: existing.pesanPelanggaran || 'Anda terdeteksi berpindah tab atau aplikasi.',
                deteksiPelanggaran: existing.deteksiPelanggaran !== false,
                alarmType: existing.alarmType || 'beep',
                alarmDurasi: existing.alarmDurasi || 3000,
                acakSoal: existing.acakSoal || false,
                durasiPerPaket: existing.durasiPerPaket !== false
            };
            db.set('ujian', normalized).write();
        }
        // ─────────────────────────────────────────────────────────────────────────
...
app.post('/login-siswa', (req, res) => {
    try {
        ...
        // ── PERBAIKAN BUG UTAMA ──────────────────────────────────────────────────
        // Cari semua paket yang cocok kelas, lalu filter berdasarkan waktu real-time
        ...
        // Ambil paket yang aktif DAN waktunya tepat sekarang
        const paketForKelas = semuaPaketKelas.find(p => isPaketAktifSekarang(p));
        if (!paketForKelas) {
            // Beri pesan yang lebih informatif
            const adaYangAktifAdmin = semuaPaketKelas.find(p => p.aktif);
            if (adaYangAktifAdmin) {
                const p = adaYangAktifAdmin;
                return res.status(400).json({
                    success: false,
                    message: `Ujian kelas ${user.kelas} dijadwalkan pukul ${p.jamMulai}–${p.jamSelesai}${p.tanggalUjian ? ' pada ' + p.tanggalUjian : ''}. Silakan tunggu.`
                });
            }
